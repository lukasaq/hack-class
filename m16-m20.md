...
### CDAH-M20L4-Self-Deleting Malware ###


Detection of Self-Deleting Malware
What Is Self-Deleting Malware?
﻿

Self-deleting malware is malware that completely removes itself from a target system post-intrusion. The MITRE ATT&CK® framework specifically classifies use of such malware as sub-technique T1070.004, “Indicator Removal: File Deletion.” The cybersecurity community collects different Indicators of Compromise (IOC), such as malware file hashes and process names, and associates them with known malicious Internet Protocol (IP) addresses. If those addresses are preemptively blocked, adversaries have a more difficult time launching attacks against vulnerable systems. By using self-deleting malware, adversaries make discovery of their infrastructure more difficult, thereby hindering investigators from finding other IOCs and increasing the number of attacks that the adversary can make. 

﻿

Examples of Self-Deleting Malware
﻿

An example of self-deleting malware is the Rombertik virus, which deletes the Master Boot Record (MBR) on Windows hosts and writes 960 million random bytes of data if an infected drive is launched on a Virtual Machine (VM). Figure 20.4-1 illustrates the Rombertik virus process.

![image](https://github.com/user-attachments/assets/3e14c8a6-5190-4f6c-b140-ac215079c509)

Another example of self-deleting malware is Jester Stealer. In 2022, adversaries sent phishing emails falsely warning Ukranians about locations to come under heavy chemical attack by Russian forces. The emails contained a malicious Microsoft Excel file that the email claimed was a list of such locations. Once opened, the Excel file began automatically running macros to download and execute the Jester Stealer malware suite. Jester Stealer can be disguised as such common file types as .txt, .jar, .ps1, .bat, .png, and .xls. 


Because most analysts investigate malware inside a virtualized environment, Jester Stealer immediately checks whether it is running in a VM and, if it is, deletes itself to prevent inspection of its code. Jester Stealer exfiltrates data from an infected host to a system of the adversary’s choosing and then deletes itself the moment it completes. Figure 20.4-2 illustrates the Jester Stealer process. Adversaries continue to update the techniques contained within the malicious code to avoid detection and exfiltrate data from newer file types and software suites.


![image](https://github.com/user-attachments/assets/8ce5a25f-438f-4140-98ec-d8732fee5573)

Detecting Self-Deleting Malware


Detecting malware that is built with anti-detection methods can be quite challenging. Typically, analysts isolate a disk in a VM to scan and inspect the host. Most sophisticated modern malware destroys itself as soon as the VM is booted or corrupts the core functions of the Operating System (OS) to the point that the VM cannot even boot. Because of this, the two primary methods for detecting self-deleting malware on a host are log data and forensic imaging data. 


Logs may contain IOCs, such as connections to external unknown IP addresses or websites, additional files being created and deleted, additional processes being launched, and registry and file changes. These IOCs can help analysts identify patterns in behavior used by the threat actor and may help pinpoint the type of malware in use. On Windows systems, Windows event logs are the best source of log information. These logs can be enhanced through the use of tools like Sysmon, a Sysinternals tool designed to provide additional context to the Windows event logs based on customized rule sets set up via a configuration file. Linux-based systems have a plethora of logging options that can be installed, such as the Debian-based syslog system. These logging tools can be tuned via configuration files, and the data can be collated in ways that make it easier to analyze and read.


With all OSs, it is best to forward logs to a Security Information and Event Manager (SIEM) or log server. By forwarding the logs to an external log server, analysts and network owners can prevent the destruction of this valuable data.


Forensic imaging through the use of such tools as Autopsy or FTK® Forensic Toolkit allows analysts to create an image of a host that can be analyzed independently of the system. These images create an exact copy of the system as it is, down to the memory level, allowing the analysis of the New Technology File System (NTFS) change journal, process history, currently running processes, files marked for deletion, and other system-related functions that may include IOCs from malware, even after deletion. Windows systems also generate prefetch files whenever an application is run for the first time. These files contain data that Windows uses to speed up the load time of the application whenever it is used and can provide forensic data about an application even after it has been deleted. Linux-based systems have a virtual file system, the /proc/ directory, where similar forensic data about currently running processes can be found, including a copy of the executable used to launch the process.


Self-deleting malware can be difficult to track. Hackers are motivated to devise anti-detection methods into their malware because better methods result in greater profits. Cybersecurity experts are likewise constantly upgrading their own tool sets to attempt to overcome the obfuscation techniques that adversaries use in the real world.

![image](https://github.com/user-attachments/assets/ed257581-78eb-48e0-9d22-d8f339673440)


-------------

Investigate Self-Deleting Malware on Windows
Investigate Self-Deleting Malware Activity
﻿

In the following scenario, a local network team has seen logs from a process called Jester that does not correlate to a known process for any software that its department uses. Analysts have been asked to review system logs to determine whether malware was on a host and, if so, whether it removed itself afterward. Investigate logs in Elasticsearch for errors that may indicate that commands were executed from a deleted process.
...
3. Select the Discover - Elastic bookmark, and log in using the following credentials:

Username: trainee@jdmss.lan 
Password: CyberTraining1!
﻿

Select Advanced and Proceed to 199.63.64.92 (unsafe) on the Your connection is not private page.

﻿

The network team has provided a timeframe of January 12, 2023, 11:50 to 12:00, for these logs. (The timeframe should be set up in Elastic already.)

﻿

4. Knowing the process name, search for process messages containing the word jester:

*process*jester*
﻿

Note the agent.hostname field to find the hostname of the affected system, ch-dc1. Also, note the timestamps of the logs associated with the Jester process: approximately 11:55:00.

﻿

5. Look through the logs to see several indicators of potentially malicious activity, including a connection to an unknown IP address, remote thread manipulation to another process, and the changing of file creation time.

﻿

6. Log in to the VM ch-dc1 using the following credentials:
...
The process listed in the Elastic logs is no longer running on this host. 

﻿

8. Select the Start button, and search for the Event Viewer tool.

﻿

9. Navigate to Applications and Services Logs > Microsoft > Windows > Sysmon > Operational. Select Filter Current Log on the right side.

﻿

10. Select logged in the drop-down menu, and select a custom range. Set the range to the following:

Events On 1/12/2023 11:50:00 AM
Events On 1/12/2023 12:00:00 PM
﻿

11. Set the event Identifier (ID) to 1, which is the Sysmon Event ID for process creation.

﻿

12. Look for the logs associated with processes created at approximately 11:55 (the time from the Elastic logs).

﻿

13. After identifying the creation of the process, create a new filter with the following time range. (The new filter should include only the time range so that it catches all events in the time period. There is no need to filter on an event ID.)

Events On 1/12/2023 11:55:00 AM
Events On 1/12/2023 12:00:00 PM
﻿

14. Identify any logs that are linked to the malicious process. To help in identifying linked activity, be alert for such things as ParentImage in the Message text.

﻿

Completion of these steps confirms that a process was created on this system that does not correspond to any currently running processes and the process is not on a list of approved applications on the host. Further analysis would include investigation of the host for potential intrusion points, such as recently installed software, additional processes launched, and changes to applications or files.


![image](https://github.com/user-attachments/assets/79514aca-057b-460a-9d8d-1a87e2100e67)

![image](https://github.com/user-attachments/assets/7a85b51a-4539-4f3c-bbb3-ed27e6cf2746)

![image](https://github.com/user-attachments/assets/920084f9-ff36-4157-8229-1628bb6e0fc8)

![image](https://github.com/user-attachments/assets/41c4a570-bfaa-4e17-8662-364c029aa2b1)

-------------------

Investigate Self-Deleting Malware on Linux
In the following scenario, a Debian-based Linux host, ch-vote1, contains logs in Elastic that identify a process called stealer. Analysts have been tasked with logging into the system and searching for evidence.
...
1. Log in to the VM ch-vote1 using the following credentials:
...
2. Run the following command to find the log containing the process creation. Analysts know that the process was previously logged to syslog and that the name of the process is stealer. The logs for the time of the intrusion can be found in the syslog.1 file.

# sudo cat /var/log/syslog.1 | grep stealer

![image](https://github.com/user-attachments/assets/317765d0-70b6-4a11-8dc2-ae74c0f06dc8)

![image](https://github.com/user-attachments/assets/9d5e7a20-c084-4e28-b3fb-41b1204fd6b7)

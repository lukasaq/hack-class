...
IDS are monitoring systems that detect suspicious activity and generate alerts. An IDS does not prevent attacks. It creates an alert and a record of the detected activity. Based on the IDS alerts, an analyst can investigate and take action.

﻿

An Intrusion Prevention System (IPS) is a monitoring system that stops traffic and activity when it detects malicious events. IPSs and IDSs require a web-based Security Information and Event Management (SIEM) interface for analysts to view the alerts and to investigate the activity. 
...
A host-based IDS (HIDS) provides visibility of a single host. It monitors network traffic to and from the machine and observes processes or logs. A HIDS can detect user activity, file modifications, configuration changes, process events, and more. 

﻿

An agent is installed on the host and reports back to a central management node where the alerts can be analyzed. A HIDS would be used to detect rootkits, file changes, services created, and malicious activity that is file-based instead of network-based. 

﻿

Figure 25.3-1 below shows a HIDS on a network map:

﻿<img width="2048" height="2048" alt="image" src="https://github.com/user-attachments/assets/d0203c6b-df92-4014-8d0c-e50fc9d2246c" />


Network-Based IDS (NIDS)


A network-based IDS (NIDS) provides visibility of all traffic flowing through the network, but lacks visibility into the internal activity of endpoints. A NIDS is a great tool for managing multiple endpoints at scale. A single NIDS can be used to manage an entire network. 


A NIDS is placed at the entrance of the network so that all traffic flows through the sensor and is examined as network traffic moves through the device. A NIDS stores its output in Packet Capture (PCAP) files and requires use of a management system where analysts can view generated alerts based on defined rules.


Figure 25.3-2 below shows a NIDS on a network map:


<img width="2048" height="2048" alt="image" src="https://github.com/user-attachments/assets/f29946fd-0c49-49f3-9a6f-347474b51af0" />

NIDS and HIDS Differences


A HIDS focuses on one host at a time and provides details about the malicious activity on a device. When the proper logs and signatures are available, a HIDS illustrates the full overview of a compromise. A NIDS, however, focuses on the entire network, and generates alerts based on network events.


For example, when the proper signatures are created, a HIDS would be able to detect the website that was accessed to download a malicious file and generate an alert. The NIDS would contain all network activity that occurred up to, and after, the download of the malicious file. 


Using both a HIDS and NIDS provides the best coverage of malicious activity in a network. It is rare that either IDS has alerts for a full attack chain of events, but all it takes is a single alert to get an analyst's attention to begin investigating. A HIDS and a NIDS records a lot of log data and network activity that can be searched and filtered afterwards to build a full picture of the malicious activity.


Figure 25.3-3 below displays a comparative overview of NIDS and HIDS:


<img width="1667" height="922" alt="image" src="https://github.com/user-attachments/assets/e5692c07-38c4-44e1-a73c-348b0500eb9b" />
...
HIDS and NIDS Deployment
HIDS Deployment
﻿

Host Intrusion Detection Systems use an agent that communicates to a management server. An agent is a program that runs on every server (or workstation) and connects to the management server to only send back alerts based on rules. 

﻿

The management server provides an interface for Host Analysts to query and view alerts. The Host Analyst must investigate whether the alerts are benign or not, as seen in Figure 25.3-4 below:


<img width="1667" height="745" alt="image" src="https://github.com/user-attachments/assets/41bdeb34-ab10-4cba-9ad7-b2bbd5148dea" />

NIDS Deployment


When implemented in a localized environment that is not in the cloud, a NIDS is a Linux server or network appliance (like a pfSense device) that contains IDS software. If the environment is based in the cloud, then a service, such as Amazon Web Service’s (AWS) GuardDuty, or Azure's firewall, can be used to provide IDS protection for a network. 


A NIDS is typically installed on the inside perimeter of a network, just past the firewall, as seen in Figure 25.3-5 below:


<img width="1667" height="834" alt="image" src="https://github.com/user-attachments/assets/e5b23108-c497-4fa5-a89d-b6ffcf6d626a" />


With this placement, all traffic that enters or exits the network passes by the IDS and is inspected. An IDS can be placed in other sections of a network as necessary to be able to monitor specific subnets, Virtual Local Area Networks (VLAN), or areas of the network that require greater visibility. Once malicious traffic is detected, it is forwarded to an IDS management device with a dashboard installed on it for security analysts to review the alerts and investigate further.

<img width="1052" height="579" alt="image" src="https://github.com/user-attachments/assets/e43848b4-caac-41cd-8e2e-914bbb531bca" />

------------------


Detection Methods
Intrusion Detection Systems use signatures and anomalies to detect malicious events. Each detection method has its strengths and weaknesses. The Host Analyst must discern which detection method to use depending on their situation.
...
Signature detection is based on a set of rules associated with known malicious behavior. Indicators of Compromise (IOC) such as ports, Internet Protocol (IP) addresses, or strings of known malware can be detected using this method of detection. 

﻿

For example, the malware Raspberry Robin uses port 8080 and a long GET request string that can be used as a signature to detect the command and control activity of an infected system.
...
Anomaly detection is based on behavior instead of specific indicators. Anomaly detection is trained on what is normal behavior, and detects activity that deviates from the baseline. This method of detection focuses on outliers that cannot be detected through a signature. An example of this is being able to detect a user logging in from a different location or time than usual.
...
Hybrid detection is the combination of both signature and anomaly detection. Using signature detection for known malware in addition to detecting odd system behavior provides the best coverage. Both signature and anomaly-based detection methods are susceptible to false-positive alerts. 

﻿

False-positive alerts, as illustrated in Figure 25.3-6 below, are alerts that are generated for benign activity. Anomaly-based detection has a higher false-positive rate than signature detection methods, but it is still a reliable detection method.

<img width="947" height="588" alt="image" src="https://github.com/user-attachments/assets/1a3feb71-690b-4c03-89e0-3cc873759286" />


-------------

Using YARA as an IDS
There are many popular Intrusion Detection Systems a Host Analyst can use, such as Snort, Suricata, and Zeek, but access to these IDSs are not always available on a mission. 
...
Yet Another Ridiculous Acronym (YARA) is a host-based malware detection system that uses signatures for detection. YARA scans through files to examine them for binary or textual values, hashes, file sizes, etc, that can be used to detect a file, regardless of file extension or header. 

﻿

Intrusion Detection v. Malware Detection
﻿

An Intrusion Detection System is constantly checking for malicious activity originating from network or file events. YARA is a malware detection utility focused on detecting malicious files that are present on a host. 

Similar to an IDS, custom rules and signatures can be created with YARA.
...
YARA is configurable, similar to an IDS, where custom rules and signatures can be created, and is different from an antivirus program in that generally AV products are owned by a vendor and signatures are not configurable. 

﻿

YARA by default is not constantly monitoring for malicious file activity on a host like an IDS is. YARA must be manually executed by a host analyst or a configured scheduled task that executes YARA against specific files or directories on a periodic basis. YARA is great for finding files that contain specific strings, values, hashes, and can be used beyond malware detection to identify files of interest such as in a forensic investigation.

﻿

Although it is nuanced to consider YARA as an IDS, in the context of missions and this module, YARA is used and referred to as an IDS. 

<img width="1064" height="622" alt="image" src="https://github.com/user-attachments/assets/60813561-33f4-4366-93a8-67332a21bc21" />
...
An accurate signature is necessary for an IDS to function properly. Signatures must be as specific as possible in order to identify malicious behavior instead of benign traffic.  

﻿

Signatures and rules require a sequence of characters unique to the malicious threat, such as one of the following items:

IP addresses or ports

Domains 

Specific GET or POST requests

User-Agents

File types, file names, hashes or contents

Hex values within known malware in conjunction with file size

Unusual strings only seen in the network traffic

For example, a Host Analyst needing to detect Raspberry Robin with Snort could create a rule to detect the following activity:

Activity to the 172.60.150.34 IP, address over port 80, with the file.msi filename. 

The Snort rule for this would look like the following:

alert tcp any any -> 172.60.150.34 80 (content:"GET", content:"/file.msi"; msg:"Raspberry Robin downloader"; sid:1001)
﻿

The above is an example of a NIDS rule. This Snort alert matches based on network traffic occurring outbound to the 172.60.150.34 IP address over port 80. The alert also contains an Hypertext Transfer Protocol (HTTP) GET request for the file.msi filename.
...
YARA complements antivirus software by allowing security teams to create their own detections. YARA can be used to detect files containing specific information, such as social security numbers, bank information, credit card numbers, and more.

﻿

The following is an example of a YARA rule created to detect a Microsoft Word document with a malicious macro:
...
rule maldoc {
	...
	condition:
		all of them and filesize >10KB and hash.sha256(0,filesize) == "972efbb0e7990a0b8404bbf9c7a57b047db169628aba7a017fd815ee5202e4d3"
}
﻿

The above is an example of a HIDS rule. The rule matches based on the size, hash, and four strings within a known malicious file. This can then be used to identify other files on a machine that contain all of these values.
...
YARA has strict syntax requirements that must be met in order for the string to be considered a rule. The necessary YARA rule keywords are as follows: 

import - Gains access to additional YARA modules that can be referenced in the rule. Import is not a mandatory field unless a module is being referenced (as in the example above) by looking up a hash value.

rule - Specifies a unique name for the rule.

strings - Matches literal strings or hex values that are found within a file. The values are assigned to a variable.

condition - Determines how many items under strings are required to be present in a file before triggering a detection.

A YARA rule must begin with the rule keyword followed by a rule identifier. YARA rules can be tested by running yara64.exe, as shown in the following command:

yara64.exe rule.yar pathtofile
﻿

The rule.yar keyword includes the YARA rule to match. The directory being searched is pathtofile. By default, YARA does not recursively search through all directories unless the parameter -r is included.

﻿

For example, the following command searches through the entire trainee directory:

yara64.exe rule.yar -r C:\Users\trainee\
﻿

Conditions 
﻿

A rule condition can be set to match on a few items or all of them.

﻿

As seen in the attached YaraRuleSample.pdf, the term all of them is used to match on all strings. The filesize keyword is used with a greater-than symbol to match on files that are greater than ten kilobytes (10KB) in size. The hash.sha256 keyword is used to match on the sha256 hash of the file.
...
Creating YARA Rules
Create and test a YARA rule to detect malware. Testing a rule ensures that it accurately detects the targeted activity, and is therefore a necessary step that Host Analysts must perform.
...
Create a YARA rule to identify the malfile2 file in the Test Files directory. Refer to the YARA documentation provided in the environment as needed. 

﻿

1. Log in to the analyst Virtual Machine (VM) with the following credentials:

Username: trainee
Password: CyberTraining1!
﻿

2. Open the mal2rule.txt file. 

﻿

The beginning of a YARA rule is provided in the mal2rule.txt file.

﻿

As seen in Figure 25.3-7 below, the section outlined in red tells YARA to look at the first 16 bits of the file for the hexadecimal values 4D 5A. In American Standard Code for Information Interchange (ASCII), this is an “MZ”, a common signature for a Portable Executable (PE) in Windows.


<img width="583" height="317" alt="image" src="https://github.com/user-attachments/assets/ea1443c4-392d-4fd3-be84-d6ade8d5bcb8" />

Keep the text editor window open.


3. Open an Administrative Command Prompt window.  


4. Run the following commands to test the signature against the sample malware files:
cd C:\Users\trainee\Desktop
yara64.exe mal2rule.txt “Test Files” 



Although the rule alerts on malfile2, it also detects malfile3, as seen in Figure 25.3-8 below:

<img width="513" height="171" alt="image" src="https://github.com/user-attachments/assets/c22a2cf7-140f-4ce8-a54f-cbdeeb5808a2" />

Since the rule is meant to only identify malfile2, it is important to identify unique characteristics of the file.


5. Open HxD as an Administrator. 


6. Select File, and then select Open. 


7. Navigate to the Test Files directory, and select malfile2.


8. Search for the unique text string in the Decoded text section that mentions Jester, as seen in Figure 25.3-9  below:

<img width="142" height="38" alt="image" src="https://github.com/user-attachments/assets/6286df47-53ff-4ebd-818e-f7b84185b86c" />

9. In the text editor for the mal2rule.txt file, change the text to match the following:
import "hash"
rule mal2 {
	strings:
		$string1 = "The Jester is here"
	condition:
...
11. Run the following command in an Administrative Command Prompt window:
yara64.exe mal2rule.txt “Test Files” 



The malfile2 file is identified.


12. In the Administrative Command Prompt window, run the following command to get the MD5 hash of malfile2:
powershell Get-FileHash \“Test Files\malfile2\” -Algorithm MD5



Use the command output to answer the following question

<img width="1118" height="577" alt="image" src="https://github.com/user-attachments/assets/74e26f3d-bc64-46f8-9e9f-a852db7bbe9f" />


---------------


### CDAH-M25L4-Tune Host Intrusion Detection System (IDS) Signatures ###

IDS Alert Types
Intrusion Detection Systems help analysts detect activity by triggering alerts for specific activity based on rules and signatures. Therefore, Host Analysts must know how to analyze alerts, verify which are accurate, and tune signatures to minimize the frequency of invalid alerts.

﻿

The four types of IDS events are as follows:

True Positive: A genuine alert that is of value for Host Analysts to thoroughly investigate.
False Positive: An alert generated by an IDS signature that turns out to be non-malicious activity. 
False Negative: When malicious activity occurs, but without an accurate signature in place to detect the activity, an alert is not generated, and the activity goes undetected.
True Negative: When there is activity that is benign and does not generate an alert. Non-malicious traffic.
Invalid Alerts
﻿

False positives and false negatives are the two types of activity that are invalid, as shown in Figure 25.4-1 below: 


<img width="1667" height="776" alt="image" src="https://github.com/user-attachments/assets/cab5311e-15b6-44da-a6e5-18ce6584614b" />


Invalid alerts need to be addressed by Host Analysts to improve the effectiveness of a Host-based Intrusion Detection System (HIDS).


When activity generates a false alert, the rule behind the false alert must be identified and examined by the Host Analyst to determine whether the rule should be modified or disabled.


False-negative events do not create alerts. When specific information is available about a threat actor, such as an Internet Protocol (IP) address, hash, or an Indicator of Compromise (IOC), a rule should be created and tested to ensure its accuracy. 
...
Risks of Invalid IDS Alerts
Host Analysts analyze alerts and decide if they should be investigated further or not. False-positive alerts cost an analyst’s time, provide no additional intelligence, and take time away from true alerts.
...
Alert fatigue occurs when an analyst is overwhelmed by the volume of alerts and their accuracy decreases as a result. 

﻿

For example, an analyst can become desensitized to alerts due to high volume and mistake malicious activity as benign. Or, an analyst may feel pressured to clear their queue and close alerts prematurely to stay afloat. Minimizing false-positive alerts allows analysts to focus their attention on real incidents and reduces alert fatigue.

﻿

Risks of False-Negative Alerts
﻿

False-negative alerts occur when malicious activity passes through an IDS without generating an alert. False-negative alerts pose great risk because they allow malicious activity to remain undetected. Since threat actors frequently change their Tactics, Techniques, and Procedures (TTP), Host Analysts must be vigilant and update IDS rulesets on a consistent basis. 
<img width="1135" height="606" alt="image" src="https://github.com/user-attachments/assets/7d3b5fb2-360a-4315-a4dc-8ffb5c1c308c" />
<img width="1125" height="718" alt="image" src="https://github.com/user-attachments/assets/c0825b2d-1d5d-43c0-be9d-973680351bc1" />


-----------------


Evaluating Invalid Alerts
When rules are created for a specific activity, such as to detect obfuscated PowerShell running on an endpoint device, it is critical that the rule is tested. Without proper testing, it can be challenging to identify rules that are close to detecting malicious behavior, but need tweaking in order to function properly. A Host Analyst should perform the necessary actions on the endpoint to trigger the alert. If the alert is not generated, then the rule or signature must be modified until the activity is detectable. 
...
One method for identifying false-negative behavior is to have a Red Team assessment or an external penetration test. The assessment team performs attacks against the network to attempt to exploit the network and hosts and documents what is being detected. 

﻿

After the assessment is complete, the analyst compares the detections to the assessment report and identifies areas that the IDS did not detect. From there, the Host Analyst can create rules for the missing detections, or modify existing rules for improved accuracy. 

﻿

Identifying Invalid Rules
﻿

False-positive alerts can be identified quickly if an analyst reviews IDS alerts on a consistent daily basis. Over time, the analyst recognizes the alerts and knows which have already been investigated and classified as false.

﻿

The steps for evaluating alerts are as follows:

View false alerts
Investigate the activity that triggered the alerts
Identify and examine the rule(s) that correspond to the alerts
Host Analysts use the collected information to determine if the IDS rule(s) can be modified to be more specific, or if the rule(s) should be disabled altogether. 

﻿

Figure 25.4-2 below illustrates the alert evaluation process of a HIDS:

<img width="1667" height="432" alt="image" src="https://github.com/user-attachments/assets/cda23abb-4758-42af-803a-c839ddc946d0" />


Scenario


Suppose an alert is triggered every day for the execution of the whoami.exe binary. Some Virtual Private Networks (VPN) applications run whoami.exe upon startup. Most IDSs detect this activity as “account discovery” or an information-gathering technique used by an adversary. However, in this scenario, the event creates a false-positive alert since it is caused by a VPN application. It is not acceptable to disable looking for whoami.exe execution since a threat actor could use that binary. Instead, the rule should filter more on the full binary path or check for the parent process that executed the binary.


Identify Invalid Alerts


An adversary placed a malware file in the Administrator user’s AppData\Roaming directory. 


Several malware samples are provided in the malwarefiles directory. The sample file named one matches the malware placed on the system. 


Using the YARA rule file malwarerule.txt, determine if the received alerts are correct.


Workflow


1. Log in to the Virtual Machine (VM) analyst with the following credentials:
Username: trainee
Password: CyberTraining1!



2. Open the malwarerule.txt file by double-clicking it.


3. Open an Administrative Command Prompt window and run the following commands to test the rule against the malware samples:
cd C:\Users\trainee\Desktop
yara64.exe malwarerule.txt malwarefiles



The malwarerule.txt rule is meant to only identify the sample labeled one. The rule itself is triggering on any file that starts with the hex code “4D 5A”, which is American Standard Code for Information Interchange (ASCII) for “MZ”. This is the file header for a Windows Portable Executable (PE). While a good number of legitimate files use this format, it is also commonly used by malware.


4. Analyze the command output and determine if the received alerts are correct.


<img width="1090" height="581" alt="image" src="https://github.com/user-attachments/assets/254b4538-d19e-486f-81de-fc31b223e09d" />
...
Tuning an IDS
IDS tuning is the process of iteratively modifying signatures and alerts for improved accuracy. 

﻿

An IDS must be tuned to remove noisy alerts, otherwise IDSs can generate too many alerts and become ineffective. Noisy alerts are false-positive alerts that trigger too often. When the alerting threshold is set too low, an IDS triggers multiple times and generates too many alerts.

﻿

Rule tuning consists of making small changes to a rule, testing and reassessing, and repeating the steps until the number of false positives is minimized, or reaches an acceptable volume. There is no defined method for configuring an IDS to detect malicious activity only. 

﻿

For example, suppose a Host Analyst needed to create a rule to detect failed logins. This activity could be caused by an attacker trying to log in, or by an authorized user who entered their password incorrectly. Both of these events need their own rule, and each rule would need to differ slightly. Otherwise, if the Host Analyst created one rule to detect failed logins, the alerting threshold would be set too low, and many false-positive alerts would trigger.

﻿

Figure 25.4-3 below illustrates the tuning process:


<img width="1667" height="694" alt="image" src="https://github.com/user-attachments/assets/21ab6f56-2a88-4e56-a7a1-1fbfb68442b6" />


Alert Frequency


The threshold of alerts corresponds with how many times an event occurs within an amount of time. For example, if a known malicious IP connects to the company's public website once a day a single alert would be sufficient, but if the IP connects to the website thousands of times in the same day, it should not generate thousands of alerts and overwhelm the analysts reviewing them. Setting a threshold to alert once in a twenty-four hour period would reduce the number of alerts significantly. 


New Rules 


IDS signatures need to be tuned when new rules are added to the ruleset. It is common for new rules to generate false alerts if those alerts were developed in a different environment. For example, a rule may trigger an alert because the network uses a non-standard port, but the port happens to be normal traffic for that particular network. Host Analysts, or detection engineers, must decide whether to disable the rule, or to modify it to exclude the portion of the rule that is causing the alerts. 


It is beneficial to have a second IDS as a backup, with all rules disabled except for new rules. This way, new rules can be evaluated for accuracy, and those that generate a large number of false alerts can be discovered and isolated quickly. This process is similar to patching a system to test it before implementing it in production.


Tuning IDS Rules


In the following lab, practice tuning an IDS. Using the one file and HxD, modify and test the YARA rule to identify the malware in the Administrator’s AppData\Roaming directory. 


Workflow


1. Open the analyst VM.


2. Open the malwarerule.txt file. 


3. Edit the rule to match the following:	
rule malware_find {
		strings:
				$string1 = ""
...
4. Open HxD as an Administrator.


5. Select File, and then select Open.


6. Navigate to the malwarefiles directory, and open the one file.


7. In HxD, analyze the Decoded Text section of the one file for string(s) that can be added to the rule in malwarerule.txt. 


Focus on text strings that are likely to be unique to the malware itself. 


8. In the malwarerule.txt file, add the string(s) to test. 


9. In an Administrator Command Prompt window, test the rule by running the following commands:
yara64.exe malwarerule.txt malwarefiles
yara64.exe malwarerule.txt C:\Users\Administrator\AppData\Roaming



Continue testing the rule until only sample one and the malware file are identified in the command outputs. More than one string may need to be added in order to accurately identify the malware. 


Use the identified malware file to answer the following questions.


<img width="1090" height="605" alt="image" src="https://github.com/user-attachments/assets/e0d45e7b-c346-455a-836c-693f17545cc2" />
<img width="963" height="189" alt="image" src="https://github.com/user-attachments/assets/fa6bd95c-c2b2-4392-9393-627bb755f787" />

<img width="1122" height="546" alt="image" src="https://github.com/user-attachments/assets/6557bad9-d0d5-43ef-b8df-20c845e14c89" />


-----------------


Impact of Tuning an IDS
The goal of IDS tuning is to create a balance of reduced false positives and to have effective rules generating true-positive alerts. 

﻿

Tuning is more than turning rules on or off. Tuning entails the process of modifying pre-existing rules to make them effective. This is accomplished by performing actions such as the following:

Adding or removing indicators
Adding new rules
Disabling rules that are no longer applicable
Routinely testing active rules to ensure that they are performing as intended
Establish a Baseline
﻿

IDSs generally house a large number of rules, which makes it difficult to know which rules must be enabled and which should be removed and disabled due to the thousands of rules that are typically on an IDS. 

﻿

One major reason why it is difficult to know which rules should be enabled or disabled in an IDS is that IDSs generally house thousands of rules. 

﻿

Host Analysts should establish a baseline of rules by determining the activity that is most important to the organization. Ensure that those rules are working first. Then over time, additional rules can be enabled, created, or modified as needed. 

﻿

Without a baseline list of rules, it becomes overwhelming to inspect every single rule while waiting for other rules to generate an alert. 

﻿

A baseline can be used to determine the following factors:

The IDS's overall effectiveness
Whether tuning has been effective
If the IDS is not working properly
It is important to disable rules that do not apply to the environment. For example, if a rule is checking for an exploit against Cisco routers, but the environment is using Palo Alto routers, then the rule can be disabled since it is not applicable to the environment.
...
During the tuning process it is important to test the rules after changing them. Depending on the details of a rule, it may be difficult to simulate the activity outlined in the rule. However, whenever simulating the activity is possible, Host Analysts should perform this crucial step. Doing so is the best way to determine if a rule is truly catching the intended behavior, or if it is misconfigured. 

﻿

Testing is also performed when a rule needs to be updated. Suppose a threat actor has been using the command and control server of 199.163.67.84 and recently changed to the IP address of 207.26.40.224. The rule needs to be updated to contain that information and to remove the old IP.


<img width="1122" height="706" alt="image" src="https://github.com/user-attachments/assets/b62d676d-3fd9-49f2-9f4b-fbd4a7849920" />
